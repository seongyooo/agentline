...
- **PLAN** — 에이전트가 스스로 세운 단계 목록을, 에이전트의 말 그대로. 끝난 것 · 지금 하는 것 · 남은 것이 구분됩니다. AgentLine은 일을 단계로 쪼개지 않습니다. 목록을 쓰는 에이전트는 이미 그 판단을 내려서 보내오고 있고, 이건 그걸 **읽는** 것이지 추론하는 게 아닙니다. `NOW`는 에이전트가 현재 단계에 붙여 보내는 현재진행형 문구를 씁니다. 그래서 어떤 도구가 도는지가 아니라 **무엇을 위한 일인지**를 말합니다. 목록을 안 쓰는 에이전트에게는 이 칸이 아예 안 뜹니다.
- **SPINNING** — 일하고 있는데 나아가지 않는 상태. 같은 파일을 계속 다시 쓰고, 같은 명령이 같은 이유로 계속 깨지고, 몇 분째 새로운 곳에 닿지 않는 경우입니다. 화면은 **센 것만** 말합니다 — `Valve.cs written 6 times`, `4 failures in the last few minutes` — 거기서 멈춥니다. 밖에서 보면 리팩터링과 무한 루프가 똑같이 생겼고, 어느 쪽인지는 당신만 알기 때문입니다. `x`로 멈추고, `esc`로 치우면 반복이 더 나빠질 때까지 다시 뜨지 않습니다. 가장 비싸면서 가장 안 보이는 실패 방식입니다. 에이전트 자신의 터미널은 이걸 볼 수 없습니다. 스크롤백에는 자기가 방금 무슨 말을 했는지에 대한 기억이 없으니까요.
...
`fakeagent`는 모든 릴리스에 `agentline`과 함께 들어 있어서, 따로 설치할 게 없습니다.
...
AgentLine이 들어갈 수 있는 모든 상태를 입력창에서 만들어 낼 수 있습니다. 스탠드인이 "그런 척"이 아니라 실제로 그 일을 하기 때문입니다.

| 이렇게 치면 | 이렇게 됩니다 |
|---|---|
| `반복해줘` | 한 파일을 계속 다시 쓰고 한 명령을 계속 실패시킵니다 → `SPINNING` |
| `권한 물어봐` | 멈추고 물어봅니다 → `y` · `n` · `a`로 답해 볼 수 있습니다 |
| `실패하게 해줘` | 실패하는 명령 |
| `테스트 돌려줘` | 성공하는 명령 |
| `리드미 써줘` | 파일을 만듭니다 → 트리가 채워집니다 |
| 그 밖에 | 편집 한 번과 빌드 한 번 |
...
- **계획 변경 이력** — 어떤 단계가 추가·삭제·재배열됐는지.
- **셀 만한 패턴 더** — 반복 말고도, 턴당 비용이 더 안 줄어들 때나 작업 목록이 멈췄을 때.
